...
export default function Search() {
    ...
    return (
        <div className='relative'>
            ...
            {query.trim() !== '' && (
                <div className="absolute top-full left-0 w-full bg-background border border-border rounded-b-lg z-10 max-h-96 overflow-y-auto">
                    {isPending ? (
                        ...
                        <div className="space-y-1 p-2">
                            <p className="text-sm text-muted-foreground px-2">Tìm thấy {results.length} kết quả</p>
                            {results.map(result => (
                                <Link key={result.report_id} href={`/scamer/${result.slug || result.report_id}`} className="block">
