...
function TraderAvatar({ trader, index }: TraderAvatarProps) {
  ...
  return (
    <Link
      href={`/${trader.slug}`}
      className="group flex w-[90px] flex-col items-center gap-2 md:w-[110px]"
      style={{ animationDelay: `${index * 0.04}s` }}
    >
      {/* Avatar */}
      <div className="relative">
        <div className="h-16 w-16 overflow-hidden rounded-2xl border-2 border-border bg-muted shadow-sm transition-all duration-200 group-hover:scale-105 group-hover:border-primary/50 group-hover:shadow-md md:h-20 md:w-20">
          {trader.avatar_url ? (
            <img
          ...
          <div
            className={`flex h-full w-full items-center justify-center bg-gradient-to-br from-primary to-primary/60 ${trader.avatar_url ? "hidden" : ""}`}
          >
        ...
        <span className="absolute -bottom-1 -right-1 rounded-full bg-background p-0.5 shadow drop-shadow">
          <VerifiedBadge role={role} size={18} />
        </span>
      </div>
...
// ─── Reusable GDV list (home + /giao-dich-vien) ───────────────────────────────
...
const GDVList = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        setTraders((tRes.data || []) as Trader[]);
  ...
  if (loading) {
    return (
      <section className="py-20 px-0 min-h-[60vh]">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center mb-12">
            <div className="h-9 w-64 rounded-lg bg-muted/50 animate-pulse mx-auto mb-3" />
            <div className="h-4 w-48 rounded-lg bg-muted/30 animate-pulse mx-auto" />
          </div>
          <div className="h-12 rounded-lg bg-muted/40 animate-pulse mb-6" />
  ...
  return (
    <section className="py-20 md:py-24 min-h-[60vh]">
      <div className="mx-auto max-w-6xl px-4">
