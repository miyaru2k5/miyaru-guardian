...
import {
    ...
    ImageIcon, Camera, Eye, EyeOff, Check, X, Loader2, Link2,
    MessageCircle,
} from "lucide-react";
...
export default function AddProductPage() {
    ...
    const onSubmit = async (data: ProductForm) => {
        ...
        try {
            ...
            toast({ title: "Lỗi thêm dịch vụ", description: err.message, variant: "destructive" });
            setSubmitting(false);
        }
    };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex-1">
                <div className="p-2 sm:p-3 grid grid-cols-1 lg:grid-cols-2 gap-3 items-start">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-3 space-y-3">
                        ...
                        <Field label="Số Zalo (tuỳ chọn)" error={errors.phone?.message}>
                            <div className="relative">
                                {/* Zalo icon: MessageCircle + Z overlay */}
                ...
                <div className="sticky bottom-0 z-20 border-t border-border bg-background/90 backdrop-blur-sm px-3 py-2.5">
                    <div className="flex items-center justify-end gap-2">
                        ...
                        <Button type="submit" className="btn-glow gap-2 min-w-[150px]" disabled={submitting}>
                            {submitting
                                ? <><Loader2 size={14} className="animate-spin" /> Đang lưu...</>
                                : <><Check size={14} /> Thêm dịch vụ</>
                            }
                        </Button>
                    </div>
                </div>
