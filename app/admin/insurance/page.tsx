...
const Insurance = () => {
  ...
  const [safePercent, setSafePercent] = useState(80); // ✅ NEW
  ...
  const fetchFund = async () => {
    ...
    if (data) {
      ...
      setSafePercent(Number(data.safe_trade_percentage ?? 80)); // ✅ lấy từ DB
    }
  };
  ...
  const percentage = fund
    ? (fund.currently_insured / Math.max(fund.total_fund, 1)) * 100
    : 0;
    ...
    if (fund) {
      await supabase
        ...
        .update({
          ...
          safe_trade_percentage: safePercent, // ✅ save
        })
        .eq("id", fund.id);
    } else {
      await supabase.from("insurance_fund").insert({
        total_fund: totalFund,
        currently_insured: insured,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Quản lý và giám sát quỹ bảo hiểm hệ thống
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Rule: Giao dịch ≤ {safePercent}% quỹ GDV
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-card border border-border">
                ...
                <p className="text-2xl font-bold text-primary">
                  {(fund?.total_fund || 0).toLocaleString("vi-VN")}đ
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-card border border-border">
                ...
                <p className="text-2xl font-bold text-foreground">
                  {(fund?.currently_insured || 0).toLocaleString("vi-VN")}đ
                </p>
              </div>
            </div>
            ...
            {isWarning && (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-destructive/10 border border-destructive/30">
                ...
                <p className="text-sm text-destructive">
                  Cảnh báo: Vượt mức an toàn {safePercent}%!
                </p>
              </div>
            )}
          </div>
        ...
        <div className="glow-border rounded-2xl p-6 h-fit">
          ...
          <div className="space-y-4">
            ...
            <Button onClick={save} className="btn-glow w-full">
              Lưu thay đổi
            </Button>
          </div>
        </div>
