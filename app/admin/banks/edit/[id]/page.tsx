...
import {
    ...
    ChevronDown, Building2, CreditCard, User, Eye, EyeOff, Save,
} from "lucide-react";
...
export default function EditBank() {
    ...
    useEffect(() => {
        ...
        const fetchBank = async () => {
            ...
            if (error || !data) {
                toast({ title: "Không tìm thấy tài khoản", variant: "destructive" });
                router.push("/admin/banks");
                return;
            }

            // Format STK với khoảng cách
            const formattedNumber = data.account_number
                ?.replace(/\D/g, "")
                .replace(/(.{4})/g, "$1 ")
                .trim() ?? "";
        ...
        fetchBank().finally(() => setLoadingData(false));
    }, [id, reset, router]);

    // Đồng bộ selectedBank khi banks + bankName đã có
    useEffect(() => {
        if (banks.length > 0 && bankName) {
            const found = banks.find((b) => b.short_name === bankName) ?? null;
    ...
    const selectBank = (bank: VietQRBank) => {
        ...
        setValue("bank_name", bank.short_name, { shouldDirty: true });
        setBankSearch("");
        setBankDropdownOpen(false);
    };
    ...
    const onSubmit = async (data: FormData) => {
        ...
        toast({ title: "Đã cập nhật tài khoản ngân hàng" });
        router.push("/admin/banks");
    };
    ...
    return (
        <div className="max-w-7xl mx-auto py-10 px-4 md:px-6">
            ...
            <div className="flex items-center gap-3 mb-10">
                ...
                <div>
                    <h1 className="text-xl font-semibold leading-tight">Chỉnh sửa tài khoản ngân hàng</h1>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        {selectedBank ? `${selectedBank.short_name} — ${selectedBank.name}` : "Cập nhật thông tin tài khoản"}
                    </p>
                </div>
            </div>
                    ...
                    <div className="bg-card border rounded-2xl p-6 sticky top-6">
                      ...
                      disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setValue("account_number", formatted, { shouldDirty: true });
                                        }}
                                        maxLength={19}
                                    />
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setValue("account_holder", e.target.value, { shouldDirty: true });
                                        }}
                                    />
                                </div>
                            ...
                            <div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => setValue("is_visible", !isVisible, { shouldDirty: true })}
    ...
    ${isVisible ? "translate-x-0" : "-translate-x-4"}`}
        ...
        ══════════════════════════════ */}
              ...
              ${isVisible
                        ...
                        {/* Dirty indicator */}
                        {isDirty && (
                            <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium bg-amber-500/8 text-amber-700 dark:text-amber-400 border border-amber-500/20">
